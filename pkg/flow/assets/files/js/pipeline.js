...
class Pipeline {
        ...
        $.get("/api/v1/status/" + encodeURI(this.pipeline),
            (data) => {
                ...
                for (var key in groups) {
                    ...
                    var cells = [];
                    if (groupContainer.attributes.embeds) {
    ...
    checkEmbed(cell, point) {
        // container types can only go into kubernetes objects
        if (cell.attributes.type != 'container.Container') {
        ...
        if (viewsBelow.length) {
            ...
            if (viewBelow && viewBelow.model.get('parent') !== cell.id) {
                ...
                if (viewBelow.model.attributes.type == 'container.Kubernetes' && cell.attributes.type == 'container.Container') {
                    cell.addTo(this.graph);
    ...
    availableCpu() {
        ...
        var cpu = 0;
        for (var node in this.lastStatus.nodes) {
            cpu += parseInt(this.lastStatus.nodes[node].cpurequests);
        }
        console.log(cpu, this.totalCpu(), (cpu / this.totalCpu()) * 100);

        // This seems wrong.
        // CPU comes out as being over-committed when summing requests.
        // This can be solved by subtracting the total from cpu before
        // calculating percentage but I'm not convinced this is really
        // the correct way to solve this problem.
        cpu = ((cpu - this.totalCpu()) / this.totalCpu()) * 100;
