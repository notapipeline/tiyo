...
func (server *Server) Init() {
	...
	server.Address = server.config.Flow.Hostname
	server.Port = fmt.Sprintf("%d", server.config.Flow.Port)
...
func (server *Server) Run() int {
	...
	index := []string{"index/index.html", "index/header.html", "index/sidebar.html", "header.html", "footer.html"}
	render.Add("index", LoadTemplates(index...))
	...
	if server.config.Flow.Cacert != "" && server.config.Flow.Cakey != "" {
		err = server.engine.RunTLS(host, server.config.Flow.Cacert, server.config.Flow.Cakey)
	} else {
...
func (server *Server) Index(c *gin.Context) {
	c.HTML(200, "index", gin.H{
		"Title": "TIYO Flow - Kubernetes cluster designer",
	})
}

// ObjectForms : Get an object properties form and data for the pipeline canvas
func (server *Server) ObjectForms(c *gin.Context) {
	var name string = c.Params.ByName("formname")

	if contains([]string{"statefulset", "daemonset", "deployment"}, name) {
		name = "objectgroup"
	}
	if name == "" {
	...
	var form *template.Template = LoadTemplates(fmt.Sprintf("objectforms/%s.html", name))
	if form == nil {
		form = LoadTemplates("objectforms/container.html")
	}

	var tpl bytes.Buffer
	if err := form.Execute(&tpl, nil); err != nil {
		c.JSON(404, "Not found")
		return
	}
