...
func (s *Server) FindUser(email string) (*User, error) {
	...
	totp, _ = api.DecryptData(totp, s.config.Flow.Passphrase)
...
func (s *Server) AddMachine(address string) string {
	...
	if key = s.FindMachine(address); key == "" {
		...
		var (
			...
			passphrase string = ""
			expiry     string = time.Now().AddDate(1, 0, 0).String()
			pBytes     []byte
			hmac       []byte
		)
		...
		// - wrap into JSON
		// - base64 encode
		pBytes, _ = api.EncryptData([]byte(s.config.Flow.Passphrase), s.config.Flow.Passphrase)
		hmac = api.Hmac([]byte(s.config.Flow.Passphrase), pBytes)
...
func (s *Server) encryptUser(user *User) User {
	...
	val, _ := api.EncryptData([]byte(user.TotpKey), s.config.Flow.Passphrase)
