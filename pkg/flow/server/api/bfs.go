...
func (b *BinFileSystem) ListFilesRecursive(path string) (results []string) {
	results = make([]string, 0)
	if !strings.HasPrefix(path, b.Root) {
		path = fmt.Sprintf("%s/%s", b.Root, path)
	}
	if items, err := AssetDir(path); err == nil {
		for _, item := range items {
			var current string = fmt.Sprintf("%s/%s", path, item)
			if i, _ := AssetInfo(current); i == nil {
				results = append(results, b.ListFilesRecursive(current)...)
				continue
			}
			results = append(results, strings.TrimPrefix(current, b.Root+"/"))
		}
	}
	return
}
