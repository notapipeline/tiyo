...
func (api *API) Get(c *gin.Context) {
	...
	if result.Message, err = api.getValue(request.Bucket, request.Child, request.Key); err != nil {
		result.Code = 500
		result.Result = "Error"
		result.Message = err
	}
...
func (api *API) getValue(bucket, child, key string) (result string, err error) {
	if err = api.Db.View(func(tx *bolt.Tx) error {
