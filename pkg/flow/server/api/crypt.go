...
func (api *API) Encrypt(c *gin.Context) {
	...
	if val, ok := request["value"]; !ok || val == "" {
		result := Result{
			Code:    400,
			Result:  "Error",
			Message: "Bad request whilst encrypting passphrase",
		}
	...
	var passphrase []byte
	if passphrase, err = EncryptData([]byte(request["value"]), api.Config.Flow.Passphrase); err != nil {
		result := Result{
			Code:    500,
			Result:  "Error",
			Message: err.Error(),
...
func (api *API) Decrypt(c *gin.Context) {
	...
	var (
		...
		ok             bool
		decoded        []byte
		assemblePhrase string = api.Config.Flow.Passphrase
	)
